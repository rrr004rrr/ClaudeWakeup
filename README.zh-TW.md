...
| **推播設定…**         | 管理飛書／Lark 機器人 webhook 網址 + 傳送測試推播   |
...
ClaudeWakeup 可在任務完成時，推播一則訊息到[飛書／Lark 自訂機器人](https://www.feishu.cn/hc/zh-TW/articles/360024984973)
（保溫 ping 也可選擇性推播）。從工具列「推播設定…」開啟，即可管理收件對象，
不必再手動編輯設定檔：

- **Webhook 網址** — 每行貼一個機器人 webhook 網址。**列出的每個網址都會收到通知**，
  因此可同時推播給**多位用戶或多個群組**。
- **傳送測試** — 立即向所有列出的 webhook 送出一則測試訊息，確認每個機器人都接好了。
  它測試的是目前輸入框裡的內容，**即使還沒儲存**也能測。
- **儲存** — 把網址寫回 `claude-wakeup.toml`（每個收件對象一行
  `feishu_hook = <url>`）。清空輸入框即關閉推播。

> 取得 webhook 的方式：在飛書／Lark 群組中新增一個**自訂機器人**，複製它的 webhook
> 網址貼進來即可。每位用戶／每個群組各自有一個機器人與網址。
...
**所有**設定的 `feishu_hook`（成功 ✅／失敗 ❌，含任務名稱、結果、時間、輸出 log 路徑），
讓你和你新增的其他用戶都能得知任務完成。可在「推播設定」視窗管理收件清單（每行一個網址），
或在此檔加入多行 `feishu_hook = <網址>`；全部清空即關閉。
